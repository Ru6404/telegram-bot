...
ADMIN_ID = 5569793273  # Ваш Telegram ID
...
async def add_client_name(message: types.Message):
    name = message.text
    await message.answer("Введите Telegram ID клиента:")
...
async def add_client_comment(message: types.Message, name, telegram_id, phone):
    ...
    conn.commit()
    await message.answer(f"✅ Клиент {name} добавлен!")
...
async def accept_request(message: types.Message):
    ...
    if result:
        ...
        conn.commit()
        await bot.send_message(telegram_id, f"✅ Ваша заявка принята, {name}!")
        await message.answer(f"Заявка #{request_id} принята.")
    else:
...
async def reject_request(message: types.Message):
    ...
    if result:
        ...
        conn.commit()
        await bot.send_message(telegram_id, f"❌ Ваша заявка отклонена, {name}.")
...
# --- Сохранение заявки и уведомление админа ---
